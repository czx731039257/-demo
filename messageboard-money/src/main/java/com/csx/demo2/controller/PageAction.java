...
public class PageAction {
    ...
    @RequestMapping("lastPageForBill")
    public String lastPageForBill(HttpServletRequest req) throws UnsupportedEncodingException {
        req.setCharacterEncoding("UTF-8");
        HttpSession session = req.getSession();
        PageBean pageBeanForLog = (PageBean)session.getAttribute("pageBeanForBill");
        pageBeanForLog.minusPageNumber();//当前页数减一
        session.setAttribute("pageBeanForBill",pageBeanForLog);
        return "redirect:"+"bill";
    }

    @RequestMapping("nextPageForBill")
    public String nextPageForBill(HttpServletRequest req) throws UnsupportedEncodingException {
        req.setCharacterEncoding("UTF-8");
        HttpSession session = req.getSession();
        PageBean pageBeanForLog = (PageBean)session.getAttribute("pageBeanForBill");
        pageBeanForLog.addPageNumber();//当前页数加一
